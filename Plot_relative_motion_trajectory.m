...
%   None (plots are displayed and optionally saved as PDF)
    ...
    x_ls = zeros(3, k); y_ls = zeros(3, k); z_ls = zeros(3, k);  % chaser axes
    x_lc = zeros(3, k); y_lc = zeros(3, k); z_lc = zeros(3, k);  % camera axes
    for i = 1:k
        ...
        % Compute camera rotation relative to LVLH
    ...
    idx_array = 1:60:k;  % subsample for clarity
    for i = idx_array
        pos = r_l(:,i)';
...
%   None (plots added to specified axes)
    ...
    for i = idx_array
        ...
        scatter3(ax, ave_xi(1,i), ave_xi(2,i), ave_xi(3,i), 30, ...
                'MarkerFaceColor', color_edge(idx,:), ...
                'MarkerFaceAlpha', 0.6, ...
                'MarkerEdgeColor', color_edge(idx,:), ...
                'LineWidth', 1.0);
    end
...
%   x_ls, y_ls, z_ls - chaser LVLH axes (3 x k)
...
%   None (plots are added to the provided axes)
    ...
    Plot_arrow3D(start, start + 1.5 * x_ls(:,idx)', colorX, 1.0);
    Plot_arrow3D(start, start + 1.5 * y_ls(:,idx)', colorY, 1.0);
    Plot_arrow3D(start, start + 1.5 * z_ls(:,idx)', colorZ, 1.0);
