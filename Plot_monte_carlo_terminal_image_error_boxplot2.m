...
%   None (displays figure)
    ...
    [~, order] = sort(arrayfun(@(h) mean(get(h,'XData')), boxes));
    boxes = boxes(order);
    ...
    ylabel('$||\boldmath{s}_i(t_\mathrm{f}) - \boldmath{s}_{\mathrm{d},i}||_2$ (px)', ...
    ...
    % Legend
    h1 = plot(NaN,NaN,'s',...
    'Color',style.neutral.light,...
    'MarkerFaceColor',style.neutral.light,...
    'MarkerSize',10,...
    'DisplayName',...
