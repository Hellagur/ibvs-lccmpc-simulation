function fig = Plot_monte_carlo_feature_trajectories3(path, N)
    ...
    above_pos = [0.13 main_pos(2) + main_pos(4) + 0.05 0.6 0.15];  % Above: same width as main, smaller height
    right_pos = [main_pos(1) + main_pos(3) + 0.05 0.11 0.15 0.6];  % Right: same height as main, smaller width
    ...
    % Load param from first file assuming consistent across all
    ...
    for i = 1:N
        ...
        % Find the first column where the condition is violated
        violation_col = size(hist.xs, 2); % Default to full if no violation
        ...
        if is_failure
            fprintf("[%d] failure.\n", i);
            counter = counter + 1;
            plot_end = violation_col+1; % Up to violation+1
            plot_step = 1;
            if plot_end < 1
                plot_end = 1; % At least plot initial if violation at start
            end
            axes_list = [ax_main, ax_zoom_above, ax_zoom_right];
        else
            plot_end = size(hist.xs, 2); % Full trajectory
            plot_step = 1;
            axes_list = [ax_main, ax_zoom_above, ax_zoom_right];
        end
    ...
    % title(ax_zoom_above, 'Zoom Above');
    % title(ax_zoom_right, 'Zoom Right');
