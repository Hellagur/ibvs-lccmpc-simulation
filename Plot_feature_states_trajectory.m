...
%   saveFig  - logical, whether to save figure
    ...
    gamma = 0.5;
    map = turbo(Nc);
    idx = round(1 + (Nc-1) * linspace(0,1,k).^gamma);
    colors_l = map(idx, :);
    ...
    %% ---- Plot position at t = 78s (optional) ----
    % idx_t = 78*2+1;
    % for i = 1:4
    %     p1 = plot(hist.xs(2*i-1,idx_t), hist.xs(2*i,idx_t), 'o', 'MarkerFaceColor','none', ...
    ...
    %% ---- Legend ----
    % legend([p3, p2, p1, p0], {'Final Position','Initial Position',['t = ',num2str((idx_t-1)/2),'s Position'],'Target Position'}, ...
