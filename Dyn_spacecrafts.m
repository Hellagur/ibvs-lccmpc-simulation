...
%   - Control inputs are applied to the chaser only.
    ...
    ss = x(13:15); ws = x(16:18);
    rs = x(19:21); vs = x(22:24);

    %% Compute chaser dynamics
    R_si    = mrp2dcm(ss);
    tGrav_s = Dyn_dis_grav(mu, Js, ss, rs);
    aJ2_s   = Dyn_dis_J2(mu, J2, Re, rs);
    [aDrag_s, tDrag_s] = Dyn_dis_drag(cD, ms, As, we, utc, matFile, rs, vs, param.d_s, R_si);
    ...
    dx(13:15) = Cfun(ss) * ws;
    dx(16:18) = Js \ (-Sfun(ws)*Js*ws + u(4:6) + tGrav_s + tDrag_s);

    % Chaser position and velocity
    dx(19:21) = vs;
    dx(22:24) = -mu*rs/norm(rs)^3 + R_si' * u(1:3) + aDrag_s + aJ2_s;
end
