function fig = Plot_accumulated_energy_compare(file1, file2, file3)
    ...
    % text(tspan(end-35), energy1(end)-1.5, sprintf('%.2f', energy1(end)), 'FontSize', 10, 'FontName', 'Times New Roman');
    % text(tspan(end-35), energy2(end)+1.0, sprintf('%.2f', energy2(end)), 'FontSize', 10, 'FontName', 'Times New Roman');
    % text(tspan(end-35), energy3(end)+1.0, sprintf('%.2f', energy3(end)), 'FontSize', 10, 'FontName', 'Times New Roman');
