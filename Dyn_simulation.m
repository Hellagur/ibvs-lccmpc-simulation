...
%   hist  - updated history structure with k+1 states
    ...
    if param.en_inputDis
        % Exponential autoregressive model for actuator errors
        Efun = @(x, alpha, sigma) alpha * x + sqrt(1 - alpha^2) * sigma .* randn(size(x));
        ...
        hist.eTH(:,k+1) = Efun(hist.eTH(:,k), param.alpha_eTH, param.sigma_eTH);
        hist.fTH(:,k+1) = Efun(hist.fTH(:,k), param.alpha_fTH, param.sigma_fTH);
        hist.bTH(:,k+1) = Efun(hist.bTH(:,k), param.alpha_bTH, param.sigma_bTH);
        hist.eRW(:,k+1) = Efun(hist.eRW(:,k), param.alpha_eRW, param.sigma_eRW);
        hist.fRW(:,k+1) = Efun(hist.fRW(:,k), param.alpha_fRW, param.sigma_fRW);
        hist.bRW(:,k+1) = Efun(hist.bRW(:,k), param.alpha_bRW, param.sigma_bRW);
    ...
    % [~, x_] = ode45(@(t,x) Dyn_spacecrafts(t, x, u_real, param), tspan, hist.sc(:,k), opts);
    ...
    s_si = x_(end,13:15)'; % chaser attitude MRPs
    w_si = x_(end,16:18)'; % chaser angular velocity
    r_s  = x_(end,19:21)'; % chaser position ECI
    v_s  = x_(end,22:24)'; % chaser velocity ECI

    % Relative transformations: LVLH and chaser-body
    [s_sl, w_sl] = sw_LVLH2CB(r_t, v_t, s_si, w_si);
    [r_l, v_l]   = rv_ECI2LVLH(r_t, v_t, r_s, v_s);
    ...
    R_sl = mrp2dcm(s_sl);
    R_st = R_si * R_ti';
    R_ct = param.R_cs * R_st;
    R_li = R_sl' * R_si;
    R_ci = param.R_cs * R_si;
    R_cl = param.R_cs * R_sl;
    ...
    w_cl = param.wcfun(w_sl);
    v_cl = param.vcfun(R_cl, v_l, w_cl);
    vc = [v_cl; w_cl];
    ...
    if isfield(param, 'Ad')
        if iscell(param.Ad)
            ...
            Ad = mat2cell(param.Ad,size(param.Ad,1),size(param.Ad,2));
            Bd = mat2cell(param.Bd,size(param.Bd,1),size(param.Bd,2));
        end
    end

    xk1 = [hist.xs(1:8,k+1); hist.vc(:,k+1)];
    xk  = [hist.xs(1:8,k); hist.vc(:,k)];
    
    dk = xk1 - Ad{1}*xk - Bd{1}*hist.u(:,k);
