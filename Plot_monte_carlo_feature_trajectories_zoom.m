...
function fig = Plot_monte_carlo_feature_trajectories_zoom(path, N)
    ...
    for ax = all_axes
            ...
            if ax == ax_zoom_above
                if param.sd(2*j) >= zoom_above_ylim(1) && param.sd(2*j) <= zoom_above_ylim(2)
                    scatter(param.sd(2*(j-1)+1), param.sd(2*j), ...
                        marker_size * 2, style.state.desired, ...
                        'filled', 'Marker', 'pentagram', ...
                        'MarkerEdgeColor', style.neutral.dark, ...
        ...
        if ax == ax_main
            ...
            xlabel('$u$ (px)', 'FontSize', 14, 'FontName', 'Times New Roman', 'Interpreter', 'latex');
            ylabel('$v$ (px)', 'FontSize', 14, 'FontName', 'Times New Roman', 'Interpreter', 'latex');
